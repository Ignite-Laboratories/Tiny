...
// Emit expresses the underlying bits of the provided binary operands according to the logical rules generated by Bits.
		...
		for i := limit; i >= 0; i-- {
			...
			case []byte:
				for ii := len(operand) - 1; ii >= 0; ii-- {
					reversed[limit-i] = any(ReverseByte(operand[ii])).(T)
				}
			case []Bit:
				for ii := len(operand) - 1; ii >= 0; ii-- {
					reversed[limit-i] = any(operand[ii]).(T)
				}
			case byte:
		...
		*/
		...
		for _, raw := range operands {
			...
			case []Bit:
				...
				switch {
				case expr._bitLogic != nil:
					out := make([]Bit, 0)
					for i, b := range operand {
						b, _ := (*expr._bitLogic)(i, b)
						out = append(out, b...)
					}
					return out
